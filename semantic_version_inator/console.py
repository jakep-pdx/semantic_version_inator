...
def get_next_ver_file_name(): #pragma: no cover
    ...
    returns next semantic version based on inputs, or 'None' type if an error occurs
    """
    if len(sys.argv) != 3:
        raise ValueError("file name and release type values required")

    return next_semantic_version.get_next_version_given_file_name(sys.argv[1], sys.argv[2])
...
def get_next_ver(): #pragma: no cover
    ...
    return next_semantic_version.get_version_given_existing_version(sys.argv[1], sys.argv[2])